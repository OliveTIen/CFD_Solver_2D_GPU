...
void GPU::GPUSolver2::initializeData() {
	/*
	1. old data -> host data
	2. host data -> device data
	*/

	initializeHostData_byOldData();
	initializeOtherHostData();
	if (GlobalPara::basic::useGPU) {
...
void GPU::GPUSolver2::initializeOtherHostData() {
	// node neighbor element count

	myint num_node = node_host.num_node;
	myint num_element = element_host.num_element;

	memset(node_host.neighbor_element_count, 0, num_node * sizeof(myint));
	for (myint iElement = 0; iElement < num_element; iElement++) {
		for (int j = 0; j < 3; j++) {
			myint iNode = element_host.nodes[j][iElement];
			node_host.neighbor_element_count[iNode]++;
		}
	}
...
void GPU::GPUSolver2::setGPUDevice() {
	if (hasSetGPUDevice) {
		LogWriter::logDebug("GPU device already set. @GPU::GPUSolver2::setGPUDevice() " __FILE__ " \n" );
		return;
	}
