...
void GPU::calculateFlux2(ElementSoA& element_device, FieldSoA& elementField_device, EdgeSoA& edge_device) {
    ...
    calculateFluxKernel2 <<<grid, block >>> (element_device, elementField_device, edge_device);


}

__global__ void GPU::calculateFluxKernel2(ElementSoA& element_device, FieldSoA& elementField_device, EdgeSoA& edge_device) {
    // TODO: Implement this kernel.
    // 未完成

}

void GPU::calculateFluxHost(ElementSoA& element_host, FieldSoA& elementField_host, EdgeSoA& edge_host, std::map<int, int>& edge_periodic_pair) {
    /*
    ...
    */
    ...
    for (int iedge = 0; iedge < edge_host.num_edge; iedge++) {
        ...
        switch (bType) {
            //对称。对欧拉方程，相当于无粘固壁
        case _BC_symmetry:
            GPU::Flux::getEdgeFlux_wallNonViscous(element_host, elementField_host, edge_host, iedge, flux);
            break;
            //无粘固壁
        ...
        case _BC_wall_adiabat:
            // TODO: 实现无滑移绝热边界条件的计算
            GPU::Flux::getEdgeFlux_wall_adiabat(element_host, elementField_host, edge_host, iedge, flux);
            break;
            //入口
        case _BC_inlet:
            GPU::Flux::getEdgeFlux_farField(element_host, elementField_host, edge_host, iedge,
        ...
        default:// 内部：bType=-1
            if (elementR != -1) {
                // 周期和内部 统一处理
                GPU::Flux::getEdgeFlux_inner_and_periodic(element_host, elementField_host, edge_host, edge_periodic_pair, iedge, flux);
            }
        }
...
void GPU::Flux::getEdgeFlux_wallNonViscous(ElementSoA& element_host, FieldSoA& elementField_host, EdgeSoA& edge_host, long idx, double* flux) {
    // 函数中idx相当于iedge
    ...
    // 计算U_L，即edge U的左极限
    REAL U_L[4]{};
    GPU::Flux::getUByXYandElementID(element_host, elementField_host, x_edge, y_edge, iElementL, U_L);
    ...
    // 计算flux
    RiemannSolve::RiemannSolve(U_L, U_R, nx, ny, length, flux,
        GlobalPara::inviscid_flux_method::flux_conservation_scheme);
}

void GPU::Flux::getEdgeFlux_wall_adiabat(ElementSoA& element_host, FieldSoA& elementField_host, EdgeSoA& edge_host, long idx, double* flux) {
    // 固壁边界。函数中idx相当于iedge
...
void GPU::Flux::getEdgeFlux_farField(ElementSoA& element_host, FieldSoA& elementField_host, EdgeSoA& edge_host, long idx, REAL* ruvp_inf, REAL* flux) {
    ...
    // 计算ruvp_L
    REAL ruvp_L[4]{};
    GPU::Physics::U2ruvp_host(U_L, ruvp_L, Constant::gamma);
    // 检查是否发散
    ...
    //根据远场边界条件修正ruvp_L
    GPU::Flux::modify_ruvpL_farField(nx, ny, ruvp_L, ruvp_inf);
    GPU::Physics::ruvp2U_host(ruvp_L, U_L, Constant::gamma);
    // 计算flux
    RiemannSolve::RiemannSolve(U_L, U_L, nx, ny, length, flux,
        GlobalPara::inviscid_flux_method::flux_conservation_scheme);
}

void GPU::Flux::modify_ruvpL_farField(const REAL nx, const REAL ny, REAL* ruvp, const REAL* ruvp_inf) {
    // 该函数根据远场边界条件修正ruvp_L
    ...
    const REAL gamma = Constant::gamma;
...
void GPU::Flux::getEdgeFlux_inner_and_periodic(ElementSoA& element_host, FieldSoA& elementField_host, EdgeSoA& edge_host, std::map<int,int>& edge_periodic_pair, long idx, REAL* flux) {
    ...
    if (edge_host.setID[idx] != -1) {
        ...
        if (cIter == edge_periodic_pair.end()) {
            ...
            GPU::Flux::getUByXYandElementID(element_host, elementField_host, x_edge_pair, y_edge_pair, iElementR, U_R);
        }
    }
    else {
        // 内部边界
        GPU::Flux::getUByXYandElementID(element_host, elementField_host, x_edge, y_edge, iElementR, U_R);
    }
...
void GPU::Flux::getUByXYandElementID(ElementSoA& element_host, FieldSoA& elementField_host, REAL x, REAL y, int elementID, REAL* U_dist) {
