...
namespace U2NITS {
	namespace Space {
		namespace Restrict {
			...
			void modifyElementFieldU2d(GPU::ElementSoA& element, GPU::FieldSoA& elementField);
			// 修正iElemnet单元场变量。对于超出范围的数据，取邻居的平均值
			void modifyElementFieldUKernel2d(GPU::ElementSoA& element, GPU::FieldSoA& elementField, int iElement);

			inline bool outOfRange(real* ruvp) {
			...
			// 限制rho和p，使其非负且不太大
			inline void restrictRhoAndP(real* ruvp) {
			...
			// 限制rho
			inline void restrictRho(real* ruvpOrU) {
			...
			// 修正U，使其满足物理规律
			inline void restrictU2d(real* U) {
				/*
				...
				*/
				...
				real rhoE_lowest = Math::EPSILON + 0.5 / U[0] * (U[1] * U[1] + U[2] * U[2]);
				U[3] = (U[3] > rhoE_lowest) ? U[3] : rhoE_lowest;
			}
			// 修正U
			inline void restrictU2d(real& rho, real& rhou, real& rhov, real& rhoE) {
				if (rho > Math::Physics::RHO_MAX)rho = Math::Physics::RHO_MAX;
				if (rho < Math::Physics::RHO_MIN)rho = Math::Physics::RHO_MIN;
				real rhoE_lowest = Math::EPSILON + 0.5 / rho * (rhou * rhou + rhov * rhov);
