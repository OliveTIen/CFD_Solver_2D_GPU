...
__global__ void TimeIntegration_1_kernel(GPU::DReal& dt, GPU::ElementSoA& element, GPU::FieldSoA& elementField) {
    // 获取id，判断是否有效
...
void TimeIntegration_1(REAL dt, GPU::ElementSoA& element_device, GPU::FieldSoA& elementField_device) {
...
void EvolveExplicitDevice_1(REAL dt, GPU::ElementSoA& element_device, GPU::FieldSoA& elementField_device, GPU::EdgeSoA& edge_device, GPU::BoundarySetMap& boundary, GPU::SDevicePara& para) {
    // 数值通量
...
__global__ void cuda_vector_divide_by_elements_with_weight_kernel(integer length, real* dist, const real* src, real weight) {
...
void cuda_vector_divide_by_elements_with_weight(integer length, real* dist, const real* src, real weight) {
    // 带权的向量对应元素相除
...
void calculateFunctionF_modify_flux_by_volume(integer num,  real* ynp_flux[4], real* element_volume) {

    for (int j = 0; j < 4; j++) {
        //for (integer ie = 0; ie < num; ie++) {
        //    real minus_one_on_volume = -1.0 / element_volume[ie];
...
void calculateFunctionF(GPU::ElementSoA& element_device, GPU::NodeSoA& node_device, GPU::EdgeSoA& edge_device, GPU::FieldSoA& ynp, GPU::BoundarySetMap& boundary, GPU::SDevicePara& para) {
...
__global__ void cuda_vector_add_with_weight_kernel(integer length, real* dist, const real* src, real weight) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i < length) {
        dist[i] += weight * src[i];
    }
...
void cuda_vector_add_with_weight(integer length, real* dist, const real* src, real weight) {
    // 带权的向量加法
    int threadsPerBlock = 256;
    int blocksPerGrid = (length + threadsPerBlock - 1) / threadsPerBlock;
...
void evolveSingleStep_timeIntegration_scale2Darray(integer num, real* U[4], real dt, real* flux[4]) {
    for (int i = 0; i < 4; i++) {
        cuda_vector_add_with_weight(num, U[i], flux[i], dt);
    }
}

void GPU::Time::evolveSingleStep_device(real dt, GPU::ElementSoA& element_device, GPU::NodeSoA& node_device, GPU::EdgeSoA& edge_device, GPU::FieldSoA& elementField_device, BoundarySetMap& boundary, SDevicePara& para) {
